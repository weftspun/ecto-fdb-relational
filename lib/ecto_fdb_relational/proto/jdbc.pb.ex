# Generated by protoc/protoc-gen-elixir from priv/protos/grpc/relational/jdbc/v1/jdbc.proto
# (vendored verbatim from FoundationDB/fdb-record-layer's fdb-relational-grpc module).
#
# This is protobuf message codecs *only* -- no `GRPC.Service`/`GRPC.Stub` generated code.
# ADR 0003 replaced the gRPC transport with an embedded-JVM one (see
# EctoFdbRelational.Native and native/), but FRL's own `FRL.execute`/`bridge.Bridge.execute`
# (native/frl_bridge) still speak these exact `grpc.relational.jdbc.v1.*` wire messages
# in-process -- they're FRL's internal request/response shape, not specific to gRPC
# transport. The `service JDBCService { ... }` block in jdbc.proto is intentionally not
# compiled in here; regenerate with plain `--elixir_out=gen_descriptors=true:...` (no
# `plugins=grpc`) per the README if you touch priv/protos/.
